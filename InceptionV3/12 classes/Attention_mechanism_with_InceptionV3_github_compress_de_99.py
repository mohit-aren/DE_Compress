...
def InceptionV3_g_compress(filt1, filt2, filt3, filt4, filt1_1, filt2_1, filt3_1, filt4_1,
    ...
    # mixed 8: 8 x 8 x 1280
    branch3x3 = conv2d_bn(x, 192, 1, 1)
    branch3x3 = conv2d_bn(branch3x3, filt1, 3, 3,
                          strides=(2, 2), padding='valid')
    ...
    # mixed 9: 8 x 8 x 2048
    #for i in range(2):
    branch1x1 = conv2d_bn(x, filt1_1, 1, 1)
...
for i in range(1,10+1):
    ...
    for j in range(0, popsize):
        ...
        arr = model_final.evaluate(X_test,y_test)
        score_trial = 0.99*arr[1]+0.01*len(v_trial)/np.sum(v_trial)
        ...
        model_final.layers[241].set_weights([filters1])
        arr = model_final.evaluate(X_test,y_test)
        score_target = 0.99*arr[1]+0.01*len(x_t)/np.sum(x_t)
...
for k in range(0,326):
    ...
    if(k not in [241, 243, 249, 250, 252, 253, 254, 255, 258, 259, 260, 261, 263, 264, 265, 266, 267, 269, 280, 281, 283, 284, 285, 286, 289, 290, 291, 292, 295, 296, 297, 298]):
        try:
...
model1.summary()
model1.save_weights('Fire_incept_pruned_weights_99.h5')
